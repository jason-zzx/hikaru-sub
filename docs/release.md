...
7. Run short Japanese transcription for every manifest-backed Faster-Whisper model, plus a >10-minute Japanese smoke for `large-v2`. In installed and portable layouts, exercise at least one smaller model, `large-v2`, and the existing `large-v3` regression without Python configured. Progress, completion, output creation, cancellation, crash recovery, restart, and the active-job gate must work without starting a Python/sidecar process or accessing the model network.
8. A missing supported model must use its independent managed confirmation/download/progress flow. Wrong revision, missing file, size/hash mismatch, partial install, or escaped path must not report ready or affect another model. Kotoba, Qwen3, Parakeet, ReazonSpeech, CUDA, and VAD must remain visible but unavailable and must never fall back to Python or another model.
