...
- 字幕编辑器（视频播放 + libass 优先字幕预览 + CSS 明示兜底 + 行内 override 标签渲染 + 样式下拉/快速参数工具栏/样式库抽屉 + 字幕列表 + 编辑面板 + 局部缩放时间轴 + 音频波形 + 撤销重做 + Aegisub 式快捷键体系（字幕导航、逐帧/边界播放头控制、Ctrl+3/4 对轴打点、Enter 提交跳转、? 键位速查）；播放时按视频帧时间同步预览；inline 模式 UI 单行展示 `译文 / 原文`）
...
- 样式可视化编辑：编辑面板支持样式下拉、字体/字号/颜色/粗体/斜体快速 override 标签插入；样式库抽屉支持新建、删除和编辑 ASS 样式的字体、颜色、边框、对齐与边距
