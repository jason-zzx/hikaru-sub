...
- 1.3 Configure context `[required · once]` — Claude Code, Cursor, OpenCode, Codex, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi, Oh My Pi, ZCode, Snow, Reasonix, Grok, Kimi Code, DeepSeek Harness (sub-agent-dispatch platforms only; inline platforms skip)
...
Sub-agent dispatch protocol applies to all platforms and all sub-agents, including native Codex `SubagentStart` context injection with child-side pull fallback, class-2 Gemini/Qoder/Copilot/Reasonix/Trae/Grok/Kimi Code/DeepSeek Harness, hook-backed ZCode/Snow, and `trellis-research`: every dispatch prompt starts with `Active task: <task path from task.py current>` before role-specific instructions. On Grok Build, use `spawn_subagent` with `subagent_type` set to the Trellis agent name (e.g. `trellis-implement`). On Kimi Code, dispatch the built-in `coder` / `explore` sub-agent with the matching `.kimi-code/skills/trellis-<role>/SKILL.md` instructions. On DeepSeek Harness, tell the child to load the matching `.dsh/skills/trellis-agent-<role>/SKILL.md` exactly once. If `trellis_wait` is available, use DSH's default continuable background mode and wait once for each child id before the next dependent gate. Without `trellis_wait`, dispatch every dependent child from the outset with `run_in_background: false`. Never simulate waiting with shell sleep, polling loops, `job_output`, repeated `list_agents`, or another long-running command.

[workflow-state:in_progress]
Tools: `trellis-implement` / `trellis-research` name sub-agent roles dispatched through the platform's sub-agent mechanism, not skills the main session loads itself. `trellis-update-spec` is a skill. `trellis-check` exists as both; prefer the Agent/role form when verifying after code changes.
On DeepSeek Harness, role instructions ship as `trellis-agent-implement` / `trellis-agent-check` / `trellis-agent-research` child-only skills under `.dsh/skills/`. Tell the child to load the matching role exactly once. If `trellis_wait` is available, use the default background mode and wait once before entering the dependent gate; otherwise dispatch every dependent child from the outset with `run_in_background: false`. Do not poll, sleep, or use `job_output` for continuable children.
...
- **DeepSeek Harness**: tell the child to load `trellis-agent-research` exactly once. If `trellis_wait` is available, use the default continuable background mode and wait once per child id before advancing. Without `trellis_wait`, dispatch each required child from the outset with `run_in_background: false`; do not poll or sleep.
...
- **DeepSeek Harness**: tell the child to load `trellis-agent-implement` exactly once. If `trellis_wait` is available, use the default background mode and wait once for that child id before Phase 2.2. Without `trellis_wait`, dispatch it from the outset with `run_in_background: false`; do not poll or sleep.
...
- **DeepSeek Harness**: tell the child to load `trellis-agent-check` exactly once. If `trellis_wait` is available, use the default background mode and wait once for that child id before Phase 3. Without `trellis_wait`, dispatch it from the outset with `run_in_background: false`; do not poll or sleep.
...
- Treat any required check that cannot run, is skipped, or exits non-zero as blocked/failed, never passed
