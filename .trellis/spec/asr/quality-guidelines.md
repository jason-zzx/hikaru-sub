...
- discovery state-machine tests prove failure lineage is append-only, a successful reviewed retry does not reset the correction budget, prior-process overlap invalidates every affected raw row, the second harness failure closes permanently, and every terminal success/failure publication rejects later model commands;
- cross-language evidence tests freeze at least one exact known-vector byte string/hash for token/parser/timeline aggregates and mutation-reject serializer key-order/separator drift before causal attribution;
...
Wrong: compare a Python `sort_keys=True` timeline hash with a C++ insertion-order JSON hash and call the mismatch parser divergence.
Correct: freeze one exact cross-language byte representation and known-vector SHA-256, then reject serializer drift before interpreting evidence differences.
