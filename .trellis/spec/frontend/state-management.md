...
- Heavy components (SubtitleList, Timeline, page containers) must **not** subscribe raw `currentTimeMs`. Subscribe `activeCueIds` instead: the derived playback-highlight set (overlaps included) maintained by `src/services/activeCueTracker.ts` (module singleton, `initActiveCueTracker()` in AppLayout). Ordinary time uses closed intervals; at the selected cue's start, cues ending exactly there are excluded while genuine crossing/shared-start overlaps remain; completed segment stops use their left-limit snapshot. The tracker writes only when membership changes — subtitle-boundary or discrete-selection frequency.
