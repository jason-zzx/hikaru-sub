# T06R GPU upstream-fallback diagnostic lock v2

## Corrected status and authorization

**Frozen after the invalid acquisition preflight and before any fallback-candidate model load.**

This is a distinct corrected identity. It supersedes `gpu-fallback-diagnostic-lock.md` only for a future acquisition request; the superseded file remains byte-for-byte frozen and bound to the invalid-preflight report. The sole correction is direct freezing of the complete expected loaded-module inventory. No production source, publisher, candidate algorithm, config, runtime binary, model, corpus, VAD, zlib, GPU, PATH, privacy, or promotion-rejection identity changed.

This lock does not authorize acquisition. Both closed cells remain blocked until independent pre-model review passes and the user grants fresh explicit model-backed acquisition approval for both rows.

| Superseded / invalid-preflight input | Bytes | SHA-256 | Meaning |
|---|---:|---|---|
| `gpu-fallback-diagnostic-lock.md` | 12,371 | `5fcfe845451f418a1e21fe8711ba9f367fdc2e13f8c2e02dd598683dde4e073a` | superseded only; preserved byte-for-byte |
...
| research/publish_whisper_gpu_quality.py | `0b4164af9997c137450fdb02c0698d3424f54c1b4a4f1450b6fb6da104385c2c` |
| research/test_publish_whisper_gpu_quality.py | `27213240d6495907e22382a215f679cba70351d018bb81e5c3d318c44849fead` |
...
The corrected identity directly freezes every module expected from the reviewed fallback runner/VAD/CUDA process. The four system/CUDA identities are the exact current values recorded by `whisper-gpu-fallback-acquisition-preflight.json`; their root roles were independently re-attested from the restricted PATH roots. The task-local entries bind the unchanged rebuilt runtime already frozen above.

| Module | Bytes | SHA-256 | Root role | Version |
|---|---:|---|---|---|
| `hikaru-asr-ctranslate2-tests.exe` | 978,944 | `1f72c282f342abc84d69f2e663d021d06b6957a44cdef83d25c44f53e5ec0e39` | `task-local-runtime-bin` | task-local executable |
| `ctranslate2.dll` | 36,974,592 | `e2d74b6f9992da14bb8c2b931983b1bcac7c9410565712cb56c5fd64f2fb6ba2` | `task-local-runtime-bin` | CTranslate2 4.8.0 |
| `hikaru_asr_tokenizer.dll` | 2,137,088 | `d0ac979b132073ba76220f47e5b77538bf03ec1dcaea01fb5d9d928762939cad` | `task-local-runtime-bin` | task-local tokenizer |
...
Any missing, additional, renamed, size-drifted, hash-drifted, version-drifted or root-role-drifted loaded module invalidates the row. `onnxruntime_providers_shared.dll` remains a required task-local runtime file but is not promoted into this actual loaded-module set unless a future clean-process attestation proves it loaded; any such change requires a new reviewed lock.
...
Before any acquisition, a reviewer must verify oracle parity, closed config/CLI, zlib/source/runtime identities, sanitized per-attempt trace validation, promotion rejection and byte determinism. A clean review permits asking for acquisition approval only.
