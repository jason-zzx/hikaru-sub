# GPU short input/runtime parity bisect lock v4

## Status and stop boundary

**Frozen Phase 3D v4 pre-model harness repair identity.** This v4 lock does not authorize acquisition, Python Silero VAD inference, CTranslate2 Whisper/large-v3 construction, encode, generate, any parity cell, parser publication, parity attribution, quality claims, production/default changes, CPU inheritance, or a formal matrix.

V4 repairs only the task-local runner defect recorded under immutable v3: `--run-whisper-short-parity` and `--short-parity-preflight` now require the closed `--vad-model` argument. The argument must resolve canonically to `silero_vad_v6.onnx` in the active isolated runtime bin and match the frozen 1,245,151-byte SHA-256 before backend construction. The backend fallback invariant is unchanged; protocol and product routing do not expose this argument.

The v1/v2/v3 locks, v2/v3 invalid evidence, completed v3 VAD artifact, reports, and earlier GPU publications remain byte-identical. The completed v3 VAD artifact is historical input only and cannot satisfy a future v4 preflight because it binds the v3 lock.
...
| v2 invalid record | 2,285 | `5c287a6b7b252284466379873bb4e2a237fda1dc0a1babc551583757ff3fc118` |
| v2 invalid report | 1,709 | `86b7cfcde55d46848ec4003feadfff9b61c7e72e5cbca8eb0bd6658fe137b78d` |
| v3 ORT import smoke | 1,156 | `3e3bbbff2aa438a662fb2a2380d6efe688072aebf7dc43a4f07a7d8ca25623e4` |
| v3 pre-model re-attestation | 997 | `ad48d75a8a85d4cdf6090cde10b4280653362d91b9876d4b754fb6e9478b38e8` |
| v3 completed VAD preflight | 3,615 | `f7201eef4c0e912eb181291038f66942d218ef7fb5e14ff86a0783f62a447246` |
| v3 invalid first-cell record | 1,670 | `808dd23c1f69e395bd759aad183e04e6b32e8c354a54f151b64c001e22882132` |
| v3 invalid report | 2,559 | `1571773487cfe39fbf33daa149e030b29c257b3dca2760c0056604a46f79509d` |
...
<!-- SHORT-PARITY-IDENTITY:BEGIN -->
{
  ...
  "sources": {
    ...
    ".trellis/tasks/08-20-native-asr-whisper-quality-revision/research/publish_whisper_short_parity_bisect.py": {
      "sha256": "9c94d18fab5a75aee5ce10fe45f049114e34a97da153c15ef78bfd026e7081ed",
      "sizeBytes": 34794
    },
    ".trellis/tasks/08-20-native-asr-whisper-quality-revision/research/test_publish_whisper_short_parity_bisect.py": {
      "sha256": "855c7446641984e33cfc9e66227846f1e43dbbf4594725d81e9702aa690c449f",
      "sizeBytes": 30052
    },
...
## V4 changed source, tool, and binary identities
...
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/generate_whisper_short_parity_inputs.py` | 5,201 | `c7cbb3188f7504d82a679cda18fb9e63377b53c2baa7883051b235ebcf3fc9d8` |
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/whisper_short_parser_oracle.py` | 11,101 | `beed74badbfdc51cb5976650b6af5dc6394e8cb924c1842a1a3218d897e2e569` |
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/whisper_short_vad_preflight.py` | 11,331 | `21a1e0a2b9911e0fed0200f9b8bbfd4582710ef53c40d237416760c82c62843d` |
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/publish_whisper_short_parity_bisect.py` | 34,794 | `9c94d18fab5a75aee5ce10fe45f049114e34a97da153c15ef78bfd026e7081ed` |
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/publish_whisper_gpu_quality.py` | 68,715 | `ece07ee742992e4315fed0635c7c56feaa1fb6e5142e912173739f647901f30c` |
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/test_publish_whisper_short_parity_bisect.py` | 30,052 | `855c7446641984e33cfc9e66227846f1e43dbbf4594725d81e9702aa690c449f` |
| `.trellis/tasks/08-20-native-asr-whisper-quality-revision/research/test_whisper_short_parity_oracles.py` | 6,056 | `5e33d8801fdbbb683c8607b899fe63196864256dccd60fd28d5f307d5f457eb3` |
| native `hikaru-asr-ctranslate2-tests.exe` | 1,117,184 | `d49c5708dc337dfbe0cf2862b4723bfa1852d4d97ac61d7cfed5583b0615fc7c` |
| native `hikaru-asr-worker.exe` | 552,960 | `d6649b54e0b7409e6e1acd6122fb29837504868af91902e29487ed238a6fc1b1` |
| native `silero_vad_v6.onnx` | 1,245,151 | `4cbf549b8326f60f80f2536d9eefeb450a9abe83365a098031c89719f1be17d2` |
| python-wheel `hikaru-asr-ctranslate2-tests.exe` | 1,117,184 | `d49c5708dc337dfbe0cf2862b4723bfa1852d4d97ac61d7cfed5583b0615fc7c` |
| python-wheel `hikaru-asr-worker.exe` | 552,960 | `d6649b54e0b7409e6e1acd6122fb29837504868af91902e29487ed238a6fc1b1` |
| python-wheel `silero_vad_v6.onnx` | 1,245,151 | `4cbf549b8326f60f80f2536d9eefeb450a9abe83365a098031c89719f1be17d2` |

## No-model evidence

Both isolated roots passed `--fallback-self-check`, `--short-parity-self-check`, and `--self-check`. Their runtime-smoke envelopes report `modelLoaded=false`; no VAD session, VAD model inference, ASR model construction, encode, generate, parity row, parser, or attribution publication occurred.

| Runtime | Smoke SHA-256 | Runtime files | Model loaded |
|---|---|---:|---|
| `native-no-cudnn` | `4537074dee333f44e1383f1a44187e6ada0baffc6a1f584cfa31f175cd80f9c3` | 7 | `false` |
| `python-wheel` | `0df1d87751e8d27f23f2a7962f98456349f33b359fda1e6eeb7432be16407a97` | 9 | `false` |

## Future acquisition command boundary

Only a fresh explicit authorization under this exact v4 lock may run Python Silero V6 preflight or any model-backed parity cell. Every future `--short-parity-preflight` and `--run-whisper-short-parity` command must pass `--vad-model <isolated-runtime-bin>/silero_vad_v6.onnx`; missing, unknown, non-runtime, escaping, renamed, size-drifted, hash-drifted, lock-drifted, or runtime-file-set-drifted inputs fail before backend construction. Future rows must publish the sanitized VAD asset identity and the publisher accepts only this exact v4 lock.

A clean v4 pre-model review permits only requesting fresh acquisition authorization. It does not authorize VAD or ASR inference and emits no parity attribution.
