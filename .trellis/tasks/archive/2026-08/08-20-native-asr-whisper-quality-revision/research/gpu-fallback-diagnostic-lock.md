# T06R GPU upstream-fallback diagnostic lock

## Status and authorization

**Prepared after Phase 3C source/oracle implementation and before any fallback-candidate model load.**

This lock does not authorize acquisition. The two commands described below remain blocked until the user separately approves model-backed diagnostic execution after independent pre-model review.
...
| research/test_publish_whisper_gpu_quality.py | `5a610a71c80c60c7967245d85721484815c6d6f8a583eea13228da373472f537` |
...
The future CLI is fixed to `--run-whisper-quality-fallback-diagnostic` plus exactly the existing cell/model/audio/output/input-lock/worker/model-identity/repeat/VAD arguments. Running either cell is currently unauthorized. No raw fallback diagnostic or result publication exists under this lock.
