...
The third branch is a truthful migration handoff, not a product qualification or GPU-required decision. It preserves unresolved large-v3/large-v2 hard gates, Candidate A's 7 long-v1 confirmed gaps, Candidate B's 1 medium confirmed gap, all remaining models as `blocked-not-run`, the `low-volume` corpus limitation, disabled native faster-whisper, and Release/default Python legacy. Because the worker seam and CPU checkpoint are complete, T07 was the immediate development step even though CPU ceiling was not proven; its purpose is iteration speed, not route qualification. T07 has now recorded `development-gpu-ready` on the declared RTX 3070: short-v1 CPU/GPU warmed median RTF `0.577901/0.062971`, locked 120s `0.591635/0.077707`, with actual driver/CUDA modules and no cuDNN. T08 therefore follows on this GPU lane. Subtitle quality does not select CPU versus GPU: CER/gap/timeline failures remain algorithm work on GPU. ORT/VAD remains excluded from T13 packaging.
...
CrispASR productization likewise starts with documented upstream behavior. Python gap detection, chunking, Japanese segmentation, backfill and refresh patterns are copied only when ground-truth evidence and authoritative implementation guidance justify them, never to achieve Python parity. T09 establishes both the shared production backend seam and an ignored-local development GPU execution path, preferring CUDA on the declared development machine and recording requested/resolved device plus loaded modules. T10/T11 use that path whenever execution is attested and repeatably faster than the paired CPU baseline; CER, gap, segmentation or alignment failures remain GPU-side algorithm iteration and never trigger CPU fallback. Final CPU and publishable GPU evidence remains separately gated.
...
5. Build the T09 CrispASR core together with its development GPU seam, then promote T10/T11 engines independently using GPU-first iteration when available.
...
- **D5:** T07 is the early CTranslate2 development CUDA lane and T09 owns the CrispASR development GPU seam; both exist to accelerate quality iteration. T07 primary identity uses CUDA 12.8 with `WITH_CUDNN=OFF`; it recorded `development-gpu-ready` with GPU/CPU warmed-median ratios `0.1090` and `0.1313` on short-v1 and the locked 120s sample. Development device selection depends only on validated availability and repeatable speedup, never subtitle quality. T14/T15 remain separate normal-numbered supply-chain and qualification deliverables, and no development-lane diagnostic artifact is publishable.
