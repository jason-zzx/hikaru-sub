...
The third branch is a truthful migration handoff, not a product qualification or GPU-required decision. It preserves unresolved large-v3/large-v2 hard gates, Candidate A's 7 long-v1 confirmed gaps, Candidate B's 1 medium confirmed gap, all remaining models as `blocked-not-run`, the `low-volume` corpus limitation, disabled native faster-whisper, and Release/default Python legacy. Because the worker seam and CPU checkpoint are complete, T07 is the immediate development step even though CPU ceiling was not proven; its purpose is iteration speed, not route qualification. T08 follows after T07 records `development-gpu-ready`, `development-gpu-unavailable` or `development-gpu-no-speedup`. Subtitle quality does not select CPU versus GPU: if GPU execution is attested and repeatably faster than the paired CPU baseline, CER/gap/timeline failures remain algorithm work on GPU. ORT/VAD remains excluded from T13 packaging.
...
- **D5:** T07 is the early CTranslate2 development CUDA lane and T09 owns the CrispASR development GPU seam; both exist to accelerate quality iteration. Development device selection depends only on attested availability and repeatable speedup over the paired CPU baseline, never subtitle quality. T14/T15 remain separate normal-numbered supply-chain and qualification deliverables, and no development-lane diagnostic artifact is publishable.
