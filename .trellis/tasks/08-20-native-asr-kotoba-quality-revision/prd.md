...
- 后续资格遵循 `native-gpu-authoritative-v1`：K3 只运行 GPU model-backed matrix，匹配 CPU route 以 `inherited-from-gpu` 继承且不产生 CPU 测量。
...
4. 同一冻结 K3 GPU identity 完成 short-v1、medium-v1、long-v2；单 case 质量失败不得截断剩余矩阵。
...
- [ ] short-v1/medium-v1/long-v2 均有 identity-valid GPU completed 或完整 structured-failure row，最终 disposition 来自完整矩阵。
- [ ] stride、ownership、dedup、progress、text conservation、timeline 和 protocol mutation tests 证明 K2 safety contract 未被弱化。
- [ ] 每个 GPU case 的 CER、S/D/I、空文本和 semantic gaps 均不劣于匹配 Python row，并通过全部绝对 GPU 硬门禁，才可发布 accepted K3 algorithm input。
